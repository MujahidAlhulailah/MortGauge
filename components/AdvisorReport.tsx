...
import { FileText, Loader2, ShieldAlert, Sparkles, Printer } from 'lucide-react';
...
export const AdvisorReport: React.FC<AdvisorReportProps> = ({ report, loading, onGenerate, comparison }) => {
  if (!report && !loading) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-white rounded-lg border border-slate-200">
        ...
        <h3 className="text-xl font-bold text-slate-900 mb-2">Financial Advisor Strategy Report</h3>
        <p className="text-slate-500 max-w-md mb-8">
          Generate a comprehensive, professional analysis of your current mortgage configuration. 
          Our AI consultant uses advanced reasoning to evaluate your strategy, opportunity costs, 
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-white rounded-lg border border-slate-200 min-h-[400px]">
        ...
        <p className="text-sm text-slate-500 mt-2 max-w-sm">
          Please wait while our AI models complex scenarios, calculates compound efficiency, and prepares your executive summary.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white h-full overflow-y-auto custom-scrollbar rounded-lg border border-slate-200 print:h-auto print:overflow-visible print:border-none">
        ...
        <div className="print:block">
          ...
          <div className="border-b-4 border-indigo-900 pb-6 mb-10 flex justify-between items-end">
            <div>
              ...
              <h1 className="text-4xl font-serif font-bold text-slate-900 leading-tight">{report?.title || "Mortgage Strategy Assessment"}</h1>
            </div>
